...
if calc_AutoCorr:
	...
	acorr = DiffCorr( exposure_diffs_pol ).autocorr()
	cor_mean = DiffCorr( exposure_diffs_pol ).autocorr().mean(0)
...
if plotting:
	...
	sp_size = 18#20
	l_pad = 10
	#[:rad_pro_mean [cntr of rad profile:-600] .shape[0]] = index + 1 of area to plot
	...
	#pypl.title('Radial Profile [from center] %s_%s_%s_(%s-sprd%s)_#%i\n' %(name,run,pdb,noisy,n_spread,N))
