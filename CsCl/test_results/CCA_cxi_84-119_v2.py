...
XCCA_Loki = False#True # True	# if True: run XCCA with Loki-pkg
XCCA_cxiLT14py = True#False # True	# if True: run XCCA with A.Martins Scripts
...
if XCCA_Loki:
	...
	if from_file_c:
		...
		calc_RadProfile = True#False#True 		# Calculate the radial Profile + Save to File
		...
		else:  calc_AutoCorr = False#True 		# only for N > 1 (ense no pattern to compare to)
		...
		if calc_RadProfile:
			...
			#print "Subplot saved as %s " %fig_name
			del fig_name

			print "shape/dim of radial_profile: ", rad_pro.shape
			radial_dim = rad_pro.shape
			del rad_pro 	# clear up memory

			

			print "Dim of Intensity Pattern: ", intensity_pattern.shape, " img[0]-variables dim: ", img[0].shape
			plt.figure(2, figsize=(8,8))
			#plt.imshow(img[0], interpolation='nearest', vmax=100)
			plt.imshow(img[0])#, vmax=100)
			cb = plt.colorbar()
			cb.set_label(label='Intensity [A.U]', weight='bold', size=12)
			plt.gca().tick_params(labelsize=14, length=9)
			#plt.show()
		#######################################################################################
		...
		if not run_with_MASK:
			...
			qmin_pix = 60#50# 	# q from origin{center-pixel}
			qmax_pix = 250#150 #250
		...
		nphi_bins_5 = [nth/5.0, 2*nth/5.0, 3*nth/5.0, 4*nth/5.0, 5*nth/5.0 ]	# div 5
		...
		# ---- Make a Polar Mask : ----
		polar_mask = Interp.nearest(m_b, dtype=bool).round()
		...
		if calc_AutoCorr:
			...
			if polar_cord:
				...
				pypl.title("Auto-Correlation [%s]" %cord_sys,  fontsize=14)
				#show()
			############### [fig.4b]  Only in Carteesian ??  Plot Sigma (Normed Auto-Correlation with ±2 std as limits) 
			subplt_ave_corrs = True#False#True 		#### Plot the 2nd plot {adapted from in // GDrive/.../scripts/ave_corrs & 'plot_a_corr.py'} ####
			if subplt_ave_corrs:
				...
				ax.set_title(r"Average of %d corrs [%s] with limits $\mu \pm 2\sigma$"%(tot_corr_count[0], cord_sys),  fontsize=14)
				
				pypl.rcParams.update({'axes.labelsize': 14, 'xtick.labelsize':'small', 'ytick.labelsize':'small'})
				show()
				############### [fig.4b] End ############### 
			fig_name = "Figure_4_Diff-Auto-Corr_SUBPLOT_(qx-%i_qi-%i)_w_Mask_%s.%s" %(qmax_pix,qmin_pix,cord_sys,frmt)
			#pypl.savefig( out_fname + fig_name)
			#print "Sub-plot saved as %s " %fig_name
		################################################################################ 
		...
		if calc_CrossCorr:				# shot[qindex]
			...
			if plt_trend_q:
				...
				xlabel(r'$q \, [\AA^{-1}]$', fontsize =16) #("q index", fontsize =16)
...
if XCCA_cxiLT14py:		## from AngularCorrelation.py {- simple script to calculate angular correlations and check convergence}
	...
	###################### Choose PLOTs: ###################
	plot_polar_mask = False#True
	plot_acorr_mask = False#True
	...
	#polarRange = [60, 250, 0, theta_half]
	...
	nq, nth = polarRange[1]-polarRange[0], 5 # Try same as in LOKI above in line 568-569 & 572; with nq = qmax_pix-qmin_pix, nth = nphi
	...
	if plot_polar_mask:
		...
		pypl.xlabel(r'$\phi$', fontsize = 18), pypl.ylabel(r'q  [pixels]', fontsize =18)#(r'$q \, [\AA^{-1}]$', fontsize =18)
		pypl.show()
		fig_name = "Figure_1_SUBPLOT_Polar_MASk_(qx-%i_qi-%i)_w_Mask.%s" %(qmax,qmin,frmt)
		#pypl.savefig( out_fname + fig_name)
		#print "Plot saved as %s " %fig_name
		#del fig_name
	######################################################################

	########## PLOT Polar MAsks [fig.2]: ################################
	if plot_acorr_mask:	# ifft (conjunate) => Ned ABS-value
		...
		pypl.xlabel(r'$\phi$', fontsize = 18), pypl.ylabel(r'q  [pixels]', fontsize =18)#(r'$q \, [\AA^{-1}]$', fontsize =18)
		pypl.show()
		fig_name = "Figure_2_MASK-ACC_(qx-%i_qi-%i)_w_Mask.%s" %(qmax,qmin,frmt)
		#pypl.savefig( out_fname + fig_name)
		#print "\ Plot saved as %s " %fig_name
		#del fig_name
	######################################################################
